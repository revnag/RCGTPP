...
# Creating Graph conditions 
       if loc!= []:
         ...
         if 'mean_dN' in pos:
           ...
           led='dN'
           rangey=[-0.05,0.05]
           fig=pos_graph(seltppPos,yax,rangey,pram,sitel,led,mycolorVal)   
         elif 'mean_dE' in pos:
            yax='mean_dE'
            mycolorVal='yellow'
            pram='Mean Position '
...
def pos_graph(possel,yax,rangey,pram,sitel,led,mycolorVal):
  fig = px.scatter(possel,template="plotly_dark",y=yax, x="DOY",
        labels={"DOY": 'Day of year 2024','mean_dN':'dN','mean_dE':'dE','mean_dH':'dH'
                },title=f'{pram} in {sitel}',)
  fig.update_layout(yaxis_title='Component in mm',legend_title=f' {led}')
